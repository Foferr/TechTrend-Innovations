...
const TablaDinamica = () => {
  ...
  const createFaqText = language === 'es' ? 'Agregar FAQ' : 'Add FAQ';
  const tableTitleText = language === 'es' ? 'Editar FAQs' : 'Edit FAQs';
  const createFaqModalTitleText = language === 'es' ? 'Crear Nuevo FAQ' : 'Create New FAQ';
  const createFaqModalQuestionPlaceholderText = language === 'es' ? 'Pregunta' : 'Question';
  const createFaqModalResponsePlaceholderText = language === 'es' ? 'Respuesta' : 'Response';
  const createFaqModelStatusOptions: { [key: string]: string[]} = {
    es: [
        'Borrador',
        'Publicado'
    ],
  ...
  const cancelCreateFAQButtonText = language === 'es' ? 'Cancelar' : 'Cancel';
  const postFAQButtonText = language === 'es' ? 'Enviar' : 'Send';
    ...
    <div>
      ...
      <div className='relative min-h-screen flex flex-col items-center bg-gray-100 py-8'>
        <div className={`relative w-11/12 bg-white p-8 rounded-lg shadow-md border-solid border border-gray-300 ${isModalOpen ? 'opacity-50' : ''}`}>
          <h1 className="text-2xl font-semibold mb-6 text-center">{tableTitleText}</h1>
          <table className="w-full text-center border-collapse">
            <thead>
              <tr>
                ...
                <th className="border-b py-4">{columnNames[language].creationDate}</th>
                <th className="border-b py-4">{columnNames[language].status}</th>
                <th className="border-b py-4">{columnNames[language].actions}</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
        ...
        {isModalOpen && (
          <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
            <div className="bg-white p-8 rounded-lg shadow-md w-1/3">
              <h2 className="text-xl font-semibold mb-4">{createFaqModalTitleText}</h2>
              <input
                type="text"
                name="question"
                value={newFaq.question}
                onChange={handleInputChange}
                placeholder={createFaqModalQuestionPlaceholderText}
              ...
              <input
                ...
                value={newFaq.answer}
                onChange={handleInputChange}
                placeholder={createFaqModalResponsePlaceholderText}
                className="w-full mb-4 p-2 border border-gray-300 rounded"
              />
              {/* <input
                type="text"
              ...
              <select
                ...
                className="w-full mb-4 p-2 border border-gray-300 rounded"
              >
                <option value="drafted">{createFaqModelStatusOptions[language][0]}</option>
                <option value="published">{createFaqModelStatusOptions[language][1]}</option>
              </select>
              <div className="flex justify-end">
                ...
                >
                  {cancelCreateFAQButtonText}
                </button>
                <button
                  onClick={handleSubmit}
                  className="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-700"
                >
                  {sendCreateFAQButtonText}
                </button>
              </div>
            </div>
          </div>
